package com.wust.endpoint.chain.quetsal.core;
...
import com.wust.endpoint.chain.quetsal.configuration.QuetzalConfig;
import com.wust.endpoint.chain.quetsal.configuration.Summary;
import com.wust.endpoint.chain.quetsal.datastructues.HyperGraph;
import com.wust.endpoint.chain.quetsal.datastructues.Trie;
import com.wust.endpoint.chain.quetsal.datastructues.TrieNode;
...
public class TBSSSourceSelection extends SourceSelection {
	...
	public Map<HyperGraph.HyperEdge, StatementPattern> hyperEdgeToStmt = new HashMap<HyperGraph.HyperEdge, StatementPattern>(); //Hyper edges to Triple pattern Map
	public List<Map<String, HyperGraph.Vertex>> theDNFHyperVertices = new ArrayList<Map<String, HyperGraph.Vertex>>(); // Maps of vertices in different DNF hypergraphs
	...
	private HyperGraph.Vertex collectVertex(Var var, String label, Map<String, HyperGraph.Vertex> v) {
		HyperGraph.Vertex resultVertex = v.get(label);
		if (null == resultVertex) {
			resultVertex = new HyperGraph.Vertex(var, label);
	...
	public void performSourceSelection(List<List<StatementPattern>> bgpGroups)
	{
		...
		{
			Map<String, HyperGraph.Vertex> v = new HashMap<String, HyperGraph.Vertex>();   //--Set (in fact) of all vertices used in our hypergraph. each subject, predicate and object of a triple pattern is one node until it is repeated
			...
			for (StatementPattern stmt : stmts)
			{
				...
				HyperGraph.Vertex sbjVertex, predVertex, objVertex;
				...
				//--------add hyperedges
				HyperGraph.HyperEdge hEdge = new HyperGraph.HyperEdge(sbjVertex, predVertex, objVertex);
				// E.add(hEdge);
...
//	}
	...
	*/
	 ...
	 */
	public Map<StatementPattern, List<StatementSource>> pruneSources(List<Map<String, HyperGraph.Vertex>> dNFHyperVertices)
	{
		for (Map<String, HyperGraph.Vertex> vs : dNFHyperVertices)
		{
			//System.out.println("--------------new DNF Graph---------");
			if (vs.size() > 3)  //---- only consider those DNF groups having at least 2 triple patterns
			{
				...
				for (HyperGraph.Vertex v : vs.values()) //vertex
				{
					...
					if (!v.inEdges.isEmpty() && !v.outEdges.isEmpty() && v.var.getValue() == null) 
					{
						...
						for (HyperGraph.HyperEdge inEdge : v.inEdges) //has hyperedges or statement patterns
						{
						...
						for (HyperGraph.HyperEdge outEdge: v.outEdges)
						{
					...
					else if (v.outEdges.size() > 1) 
					{
						...
						for (HyperGraph.HyperEdge outEdge : v.outEdges) //has hyperedges or statement patterns
						{
							StatementPattern stmt =  hyperEdgeToStmt.get(outEdge);
							StatementPatternSourceDescriptor d = handleStatement(stmt, subjectSetProjection, src -> getFedSumDMatchingSbjAuthorities(stmt, src), stmtToPrefixes, prefixIntersectionSet);
							sd.add(new StatementDescriptor(stmt, d, subjectSetProjection));
...
//	}
	...
	*/
	 ...
	 * @return List of authorities
	 */
	public Set<String> FedSumD_getMatchingObjAuthorities(StatementPattern stmt, StatementSource src, HyperGraph.Vertex v)
	{
	...
	*/
	 ...
	 * @return SPARQL query
	 */
	public String getFedSumObjAuthLookupQuery(StatementPattern stmt, String endPointUrl, HyperGraph.Vertex v)
	{
		String queryString = null;
		String s = null, p = null, o = null, sa = null, oa = null;
		if (stmt.getSubjectVar().getValue() != null)
