...
import com.wust.endpoint.chain.quetsal.core.Cardinality;
import com.wust.endpoint.chain.quetsal.core.CardinalityVisitor;
...
public class RestarterIteration<E> extends LookAheadIteration<E, QueryEvaluationException> {
	...
	class CostVisitor extends AbstractQueryModelVisitor<RuntimeException>
	{
		...
		double cost;
		TopKSourceStatementPattern.Entry curEntry;
		double nBindingsCfg;
		
		void reset(TopKSourceStatementPattern.Entry e) {
			cost = 0;
			curEntry = e;
	...
	protected E getNextElement() {
		while (true) {
			...
			List<TopKSourceStatementPattern.Entry> entries = jr.getEntries();
			...
			{
				TopKSourceStatementPattern.Entry e = entries.get(v);
